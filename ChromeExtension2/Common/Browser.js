...
var Browser = 
{
   ...
   IsMobilePhone: function()
   {
	...
	_EnvironmentType: function()
	{
		if (Browser.environmentType == null)
		{
			if (typeof chrome !== 'undefined' && chrome.extension && chrome.extension.getBackgroundPage && chrome.extension.getBackgroundPage() === window)
			{
				Browser.environmentType = 'BACKGROUND';
			}
			else if (typeof chrome !== 'undefined' && chrome.extension && chrome.extension.getBackgroundPage && chrome.extension.getBackgroundPage() !== window)
			{
				Browser.environmentType = 'POPUP';
			}
			else if (typeof chrome === 'undefined' || !chrome.runtime || !chrome.runtime.onMessage)
			{
				Browser.environmentType = 'WEB';
			}
			else
			{
				Browser.environmentType = 'CONTENT';
			}
		}
		
		return Browser.environmentType;
	},

	IsExtension: function()
	{
		return Browser._EnvironmentType() == 'BACKGROUND' || Browser._EnvironmentType() == 'CONTENT';
	},
	
	IsExtensionContent: function()
	{
		return Browser._EnvironmentType() == 'CONTENT';
	},
	
	IsExtensionBackground: function()
	{
		return Browser._EnvironmentType() == 'BACKGROUND';
	},
	
	CanAccessParent: function()
	{
// This should work but in Chrome it doesn't work for local environment (developers) so switched to the below.
//		return Window.frameElement != null;
		return window.parent != undefined && window.parent != window && window.parent.location.hostname == window.location.hostname;
	},
	...
	{
		if (window.document.webkitVisibilityState != undefined)
			return window.document.webkitVisibilityState == 'visible';
		else if (window.document.visibilityState != undefined)
